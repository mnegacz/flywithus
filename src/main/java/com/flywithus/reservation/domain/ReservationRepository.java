...
class ReservationRepository {
    ...
    ReservationRepository(ReservationRepositoryPort reservationRepositoryPort, ClientRepository clientRepository, FlightRepository flightRepository, ReservationFactory reservationFactory) {
    ...
    Reservation find(ReservationId id) {
        ...
        Optional<Discount> discount = Optional.ofNullable(dto.getDiscount())
                .map(discountDTO -> Discount.of(Percent.of(discountDTO.getPercent())));

        return reservationFactory.createReservation(id, client, flight, dateTime, numberOfPeople, discount, status);
    }
