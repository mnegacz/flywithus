...
public class UserAuthenticationProvider implements AuthenticationProvider {
    ...
    private UsernamePasswordAuthenticationToken createToken(Authentication authentication, UserDTO userDTO) {
        List<SimpleGrantedAuthority> authorities = ImmutableList.of(new SimpleGrantedAuthority(ROLE_CLIENT));
        UsernamePasswordAuthenticationToken token = new UsernamePasswordAuthenticationToken(userDTO, authentication.getCredentials(), authorities);
        token.setDetails(authentication.getDetails());
        return token;
    }

    private boolean passwordMatches(Authentication authentication, UserDTO userDTO) {
        return Arrays.equals(authentication.getCredentials().toString().toCharArray(), userDTO.getPassword());
    }
